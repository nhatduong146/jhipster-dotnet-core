...
- `src/JhipsterExample/ClientApp/` - Web Application folder
...
```bash
dotnet run --verbosity normal --project ./src/JhipsterExample/JhipsterExample.csproj
```

and

```bash
dotnet run --verbosity normal --project ./src/client/JhipsterExample.Client/JhipsterExample.Client.csproj
```
...
```bash
dotnet watch --project ./src/JhipsterExample/JhipsterExample.csproj run --verbosity normal
```

For the frontend

```bash
dotnet watch --project ./src/client/JhipsterExample.Client/JhipsterExample.Client.csproj run --verbosity normal
```
...
To build the artifacts and optimize the jhipsterExample application for production, run:

```
cd ./src/JhipsterExample
rm -rf ./src/JhipsterExample/wwwroot
dotnet publish --verbosity normal -c Release -o ./app/out ./JhipsterExample.csproj
```

The `./src/JhipsterExample/app/out` directory will contain your application dll and its depedencies.
...
3. Run ``dotnet sonarscanner begin /d:sonar.login=admin /d:sonar.password=admin /k:"JhipsterExample" /d:sonar.host.url="http://localhost:9001" /s:"`pwd`/SonarQube.Analysis.xml"``
...
```bash
docker build -f ./Dockerfile-Back -t jhipsterexample .
```

```bash
docker build -f ./Dockerfile-Front -t jhipsterexample-front .
```

Then run:

```bash
docker run -p 8080:8080 jhipsterexample
```

```bash
docker run -p 8081:80 -e ServerUrl=https://localhost:8080 jhipsterexample-front
```
